...
export default function StepExport() {
  ...
  const { calculatedGrid, boardPieces, cellColors, palette, dimensions, quiltName, setQuiltName } = useQuiltStore()
  ...
  return (
    <div>
      ...
      {/* ── Quilt name input (hidden on print) ────────── */}
      <div className="export-actions" style={{ marginTop: 0, paddingTop: 0, borderTop: 'none' }}>
        <div className="field" style={{ maxWidth: 320 }}>
          <label className="field-label" htmlFor="quilt-name">Quilt name</label>
          <input
            id="quilt-name"
            type="text"
            className="field-input"
            placeholder="e.g. Summer Stars"
            value={quiltName}
      ...
      <div className="print-section">
        ...
        <div className="print-header">
          <span className="print-title">{quiltName || 'Patchwork'}</span>
        ...
        {summary.fabrics.length > 0 && (
          <div className="print-block">
            ...
            <div className="print-fabric-list">
              {/* Header row */}
              <div className="print-fabric-row print-fabric-row--header">
                <div style={{ width: 12, flexShrink: 0 }} />
                <span style={{ flex: 1 }}>Fabric</span>
                <span style={{ minWidth: 72, textAlign: 'right' }}>Cut squares</span>
                <span style={{ minWidth: 56, textAlign: 'right' }}>Yardage</span>
              </div>
              {summary.fabrics.map((f) => (
                <div key={f.colorId ?? '__default'} className="print-fabric-row">
                  ...
                  <span className="print-fabric-cells">{f.cells}</span>
