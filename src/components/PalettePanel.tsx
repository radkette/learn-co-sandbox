...
export function PalettePanel() {
  ...
  const usedCount = (id: string) =>
    Object.values(cellColors).filter((v) => v === id).length

  const isEraser = selectedPaletteColorId === null

  return (
    <div className="palette-panel">
      <p className="text-eyebrow" style={{ marginBottom: 12 }}>Color Palette</p>
      ...
      <p className="field-hint" style={{ marginTop: 12 }}>
        Select a color, then click any placed piece to paint it. Use the eraser to remove color.
      </p>
    </div>
  )
}
